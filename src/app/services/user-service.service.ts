...
export class UserService {
  ...
  public ajoutReclamation(reclamation:any,user_id :any): Observable<any> {
    ...
    return this.http.post(USERS_API + 'ajout-reclamation',{
  ...
  public ajoutModifPhotoProfil(photo):any {
       this.http.post(USERS_API +'photo-profile',photo).subscribe(response => {
...
public verifNbFeedbackPerApprenantpersession(user_id,session): Observable<any>{

  const params = new HttpParams().set('user_id', user_id).set('session_id', session);

    httpOptions = {
      headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
      params
    };
    return this.http.get(USERS_API + 'verifNbFeedbackPerApprenantpersession', httpOptions);
}
